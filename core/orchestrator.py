...
class ModelOrchestrator:
    """Управляет переключением моделей в зависимости от потребностей системы.

    Стратегия:
    - LLM загружена постоянно (единая точка входа)
    - Logic загружается по запросу LLM через [LOGIC_REQUEST]
    - Переключение минимизировано за счёт асинхронных вызовов
    """
    ...
    async def activate_llm(self) -> None:
        """Активировать LLM (основная модель, не выгружается)."""
    ...
    async def activate_logic(self) -> None:
        """Активировать Logic ядро при запросе от LLM."""
        await self._switch_to(
    ...
    async def _switch_to(
        ...
        """Переключиться на требуемую модель."""
        ...
        if already_loaded:
            # Модель уже в памяти, просто меняем роль
            ...
            await self._event_log.log(
                channel="info",
                source="orchestrator",
                message=f"Модель {label} уже в памяти",
                payload={
                    "model": model_key,
        ...
        # Нужно загрузить новую модель
        self._active_role = ActiveRole.SWITCHING
        await self._event_log.log(
            channel="info",
            source="orchestrator",
            message=f"Переключение на {label}",
        )
        ...
        if self._loaded_instance_id:
            try:
                ...
                await self._event_log.log(
                    channel="error",
                    source="orchestrator",
                    message=f"Ошибка выгрузки текущей модели: {exc}",
                )
            self._loaded_instance_id = None
            self._loaded_model = None

        # Выгрузить остальные модели для освобождения памяти
